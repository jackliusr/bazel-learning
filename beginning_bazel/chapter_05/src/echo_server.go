...
func main() {
	...
	if err != nil {
		log.Panicln("Cann't accept a connection! Error: " + err.Error())
	}
	log.Println("Receiving a new connection")
	defer connection.Close()
	defer log.Println("Connection new closed.")

	buffer := make([]byte, 2048)
	size, err := connection.Read(buffer)
	if err != nil {
		log.Panicln("Cannot read from the buffer! Error: " + err.Error())
	}
	data := buffer[:size]
